#!/usr/bin/env bash
# Action entrypoint (contexts = ["pane"]): hotkey for "paste review into the
# origin agent's input, let the human press Enter".
set -euo pipefail
...
main() {
  ...
  log_info "Opening tuicr review pane (paste mode) for $repo_dir"
  open_review_pane "paste" "$origin_pane" "$repo_dir"
}
