...
3. `review-pane.sh` runs *as* that new pane's controlling process: snapshots `tuicr review list`, runs `tuicr -w` interactively (the human reviews here), snapshots the list again, figures out which session it just touched (`select_touched_session`), reads back `tuicr review comments`, formats them (`format_review_comments`), and dispatches to the origin pane — `herdr pane send-text` (paste) or `herdr agent prompt` (submit) — then closes itself.
